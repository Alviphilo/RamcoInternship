...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        $stmt = $conn->prepare("INSERT INTO servers 
            (server_name, server_ip, category, sub_category, purpose, allocated_date, server_status, surrendered_date, owner, priority, backup, commvault_backup, backup_status, last_backup_date, remarks)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        $stmt->bind_param("ssssssssissssss", $name, $ip, $category, $sub_category, $purpose, $allocated_date, $status, $surrendered_date, $owner, $priority, $backup, $commvault, $backup_status, $last_backup, $remarks);
        $stmt->execute();
